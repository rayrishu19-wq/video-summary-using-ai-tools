    ...
    try {
        ...
        if (error instanceof Error && error.message.includes("timeout")) {
            res.status(408).json({
                ...
                details: "Video processing took too long. Please try with a shorter video or try again later."
            });
        }
        else {
            res.status(500).json({
...
app.listen(PORT, () => {
    console.log(`🚀 Video Summarizer Web App running on http://localhost:${PORT}`);
